...
import { Loader2, Search, RefreshCw, Phone, Building2, MessageCircle } from "lucide-react";
...
export default function WhatsAppInboxPage() {
  ...
  const grouped = Object.values(
    filtered.reduce((acc: Record<string, Reply>, r) => {
      if (!acc[r.phone] || new Date(r.receivedAt) > new Date(acc[r.phone].receivedAt)) {
        acc[r.phone] = r;
      }
      return acc;
    }, {})
  ...
  const threadMessages = selected
    ? replies.filter((r) => r.phone === selected.phone).sort((a, b) => new Date(a.receivedAt).getTime() - new Date(b.receivedAt).getTime())
    : [];
      ...
      <div className="flex h-full overflow-hidden">
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b-2 border-black bg-[#25D366]">
            ...
            <button onClick={fetchReplies} className="ml-auto text-white/80 hover:text-white">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-3 px-6 text-center">
                <FontAwesomeIcon icon={faWhatsapp} className="text-4xl text-gray-200" />
                <p className="text-sm text-muted-foreground">No replies yet.</p>
                <p className="text-xs text-muted-foreground">Customer replies will appear here.</p>
              </div>
            ) : (
              grouped.map((r) => (
                <button
                  ...
                  className={cn(
                    ...
                    selected?.phone === r.phone && "bg-[#25D366]/10 border-l-4 border-l-[#25D366]"
                  )}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold truncate">{r.leadName || r.phone}</p>
                      <p className="text-[10px] text-muted-foreground shrink-0">
                        {format(new Date(r.receivedAt), "h:mm a")}
                      </p>
                    </div>
                    <p className="text-xs text-muted-foreground truncate mt-0.5">{r.messageText}</p>
                  </div>
                </button>
              ))
            )}
        ...
        {selected ? (
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b-2 border-black bg-white shrink-0">
              ...
              <div>
                <p className="text-sm font-bold">{selected.leadName || selected.phone}</p>
            ...
            <div className="px-4 py-3 border-t-2 border-black bg-white shrink-0">
              <div className="flex items-center gap-2 bg-[#FFF3CD] border border-[#FFCC00] rounded-lg px-3 py-2">
                ...
                <p className="text-xs text-gray-700">
                  To reply, open the lead and use <strong>Send WhatsApp Message</strong> button.
                  Free text replies work within 24h of last customer message.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="hidden md:flex flex-1 items-center justify-center bg-[#f0f2f5]">
            <div className="text-center">
              <FontAwesomeIcon icon={faWhatsapp} className="text-7xl text-gray-200 mb-4" />
              <p className="text-sm font-medium text-gray-400">Select a conversation to view</p>
            </div>
          </div>
        )}
      </div>
