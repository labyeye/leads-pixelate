...
export default function ConsolePage() {
  ...
  const hasFilters =
    search || filterAction !== "ALL" || filterModule !== "ALL" || filterDate;
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-0 border-b-2 border-black shrink-0">
            {[
              ...
              {
                label: "FAILED LOGINS",
                value: stats.loginFails,
                color: stats.loginFails > 0 ? "bg-red-500" : "bg-white",
                text: stats.loginFails > 0 ? "text-white" : "text-black",
              },
            ].map((s, i) => (
              <div
                key={i}
                className={cn(
                  "flex flex-col justify-center px-5 py-3 border-r-2 border-black last:border-r-0",
                  s.color,
                )}
              >
                <p className={cn("text-2xl font-black font-mono", s.text)}>
                  {s.value?.toLocaleString() ?? 0}
                </p>
                <p
                  className={cn(
                    "text-[10px] font-bold font-mono tracking-widest opacity-70",
                    s.text,
                  )}
                >
                  {s.label}
              ...
              <SelectContent>
                ...
                {Object.keys(ACTION_CONFIG).map((a) => (
                  <SelectItem key={a} value={a} className="font-mono text-xs">
                    {ACTION_CONFIG[a].label.toUpperCase()}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              {hasFilters && (
                <button
                  onClick={clearFilters}
                  className="text-xs font-mono text-[#024BAB] underline"
                >
                  CLEAR FILTERS
                </button>
              )}
            ...
            <table className="w-full text-xs font-mono border-collapse">
              <thead className="sticky top-0 z-10">
                <tr className="bg-black text-white">
                  ...
                  <th className="text-left px-4 py-2.5 font-bold tracking-widest w-28">
                    ACTION
                  </th>
                  <th className="text-left px-4 py-2.5 font-bold tracking-widest w-24">
                    MODULE
                  </th>
                  <th className="text-left px-4 py-2.5 font-bold tracking-widest">
                    USER
                  </th>
                  <th className="text-left px-4 py-2.5 font-bold tracking-widest">
                    DESCRIPTION
              ...
              <tbody>
                {logs.map((log, idx) => {
                  ...
                  const mc = MODULE_CONFIG[log.module] || {
                    Icon: Activity,
                    color: "text-gray-500",
                  };
