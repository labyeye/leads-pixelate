...
export function AutopilotPanel({ toast }: { toast: any }) {
  ...
  const toggle = (enabled: boolean) => {
    ...
    return run(
      ...
      enabled
        ? "Autopilot is on — your first posts are being prepared"
        : "Autopilot paused",
      () => setToggling(false),
    );
  };
  ...
  const banner: { tone: Tone; text: string } =
    ent.state === "trial" && ent.endsAt
      ? {
          tone: "blue",
          text: `Free trial — ${daysLeft(ent.endsAt)} day(s) left (ends ${fmtDate(ent.endsAt)}). After that Autopilot is included in your NestLeads plan.`,
        }
      : ent.state === "paid" && ent.endsAt
        ...
        : ent.state === "expired"
          ? {
              tone: "red",
              text: `Your Autopilot access has ended. Choose a NestLeads plan to keep Autopilot running.`,
            }
          : {
              tone: "blue",
              text: `Free for ${status.trialDays} days, then it comes with your NestLeads plan. Turn it on to start your trial.`,
            };
  const toneClass = {
  ...
  return (
    <div>
      <div className="space-y-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Claude studies your business and plans the posts, Gemini writes
              the captions and creates the images, and Claude approves each one
              before it goes live. You do nothing.
            </p>
          </div>
        </div>
        ...
        <div
          className={`flex items-center justify-between gap-3 rounded-lg border p-3 text-sm ${toneClass}`}
        >
          <span>{banner.text}</span>
          {ent.state !== "none" && (
            <Button asChild size="sm">
              <Link to={ent.state === "paid" ? "/ai-usage" : "/billing"}>
                {ent.state === "paid" ? "View plan & usage" : "Choose a plan"}
              </Link>
            </Button>
          )}
        </div>
        ...
        <div className="flex items-center justify-between rounded-lg border-2 border-black p-4 bg-background">
          ...
          <Switch
            ...
            disabled={
              toggling ||
              (noAccounts && !settings.enabled) ||
              (ent.state === "expired" && !settings.enabled)
            }
        ...
        {status.running && (
          <div className="flex items-center gap-2 text-sm text-blue-700">
            <Loader2 className="w-4 h-4 animate-spin" /> Generating your posts…
        ...
        {status.lastError && !status.running && (
          <div className="flex gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
            <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
            Last run failed: {status.lastError}. It retries automatically, or
            use Run now.
          </div>
        )}
        ...
        <div className="rounded-lg border-2 border-black p-4 space-y-4 bg-background">
          <div className="grid sm:grid-cols-2 gap-4">
            {settings.schedule?.times?.length ? (
              <div className="space-y-1.5">
                <Label>When to post</Label>
                <p className="text-sm rounded-md border px-3 py-2 bg-muted/40">
                  {settings.schedule.times.join(" & ")} (India time). Change it
                  under Brand &amp; posting plan below.
                </p>
              </div>
            ) : (
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm({ ...form, postsPerDay: Number(v) })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
          ...
          {status.accounts.length > 0 && (
            <div className="space-y-2">
              <Label>Post to</Label>
              <p className="text-xs text-muted-foreground">
                Leave all unticked to use every connected account.
              </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                During the free trial every post waits for your approval. After
                that Autopilot runs on its own, unless this is on.
              </p>
            </div>
            <Switch
              checked={form.reviewFirst}
              onCheckedChange={(v) => setForm({ ...form, reviewFirst: v })}
            />
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground">
          <span className="flex items-center gap-1.5">
            ...
            {status.lastRunAt &&
              ` · last run ${new Date(status.lastRunAt).toLocaleString("en-IN")}`}
          </span>
        ...
        <p className="text-xs text-muted-foreground">
          Every post Autopilot creates is listed here, and in the Posts and
          Calendar tabs of Social Media Planner.
        </p>
      </div>
    </div>
  );
