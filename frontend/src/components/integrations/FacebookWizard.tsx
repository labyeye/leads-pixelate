  ...
  const handleSaveFilter = async (cp: {
    ...
    try {
      ...
      toast({
        ...
        description: `Saved for ${cp.pageName}`,
      });
    } catch (err: unknown) {
      toast({
  ...
  const handleLoginWithFacebook = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          (err as Error).message || "Could not get Facebook login URL",
        variant: "destructive",
      });
                ...
                <div key={cp.pageId} className="border-2 border-black bg-white">
                  ...
                  {editingFilterPageId === cp.pageId && (
                    <div className="border-t-2 border-black p-3 bg-[#FFFBEB] space-y-3">
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Only capture leads from these cities/states. Leave empty
                        to capture from everywhere.
                      </p>
                      <div className="flex gap-2">
                        <input
                      ...
                      {editFilterValues.length > 0 ? (
                        <div className="flex flex-wrap gap-1.5">
                          {editFilterValues.map((s) => (
                            <span
                              key={s}
                              className="flex items-center gap-1 border-2 border-black px-2 py-0.5 bg-[#EFF6FF] text-xs font-bold"
                            >
                              {s}
                              <button
                                onClick={() =>
